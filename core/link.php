...
class link
{
	 ...
	 */
	function __construct(\phpbb\db\driver\driver $db, \phpbb\config\config $config, \phpbb\template\template $template, \phpbb\user $user, \phpbb\controller\helper $helper, \phpbb\request\request $request, \phpbb\auth\auth $auth, $container, $dir_path_helper, $root_path, $php_ext)
	{
		$this->db			= $db;
		$this->config		= $config;
