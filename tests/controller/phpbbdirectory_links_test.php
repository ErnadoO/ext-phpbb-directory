...
class phpbbdirectory_links_test extends controller_base
{
	...
	public function get_controller($user_id = 0)
	{
		...
		$this->user->data['is_registered']	= ($this->user->data['user_id'] != ANONYMOUS && ($this->user->data['user_type'] == USER_NORMAL || $this->user->data

			['user_type'] == USER_FOUNDER)) ? true : false;
	...
	public function edit_link_data()
	{
		return array(
			...
			array(1, 5, 3, 200, 'add_site.html'),
			array(1, 5, 2, 200, 'add_site.html'),
		);
	}
	...
	{
		return array(
			array(1 ,1, 403, 'DIR_ERROR_NOT_AUTH'),
			array(2 ,1, 403, 'DIR_ERROR_NOT_AUTH'),
			array(3 ,1, 403, 'DIR_ERROR_NOT_AUTH'),
		);
	}
