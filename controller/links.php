	...
	*/
	private function _data_processing($cat_id, $link_id = 0, $mode = 'new')
	{
		...
		if (!$error)
		{
			...
			if ($this->description)
			{
				generate_text_for_storage($data_edit['link_description'], $data_edit['link_uid'], $data_edit['link_bitfield'], $data_edit['link_flags'], (bool) $this->config['allow_bbcode'], (bool) $this->config['allow_post_links'], (bool) $this->config['allow_smilies']);
			}
